...
void ViewController::addAisObject(const Handle(AIS_InteractiveObject) &
                                  theAisObject) {
...
void ViewController::preDraw_() {
  ...
  if (newRenderWidth != internal_->renderWidth ||
      newRenderHeight != internal_->renderHeight) {
    ...
    internal_->offscreenFBO->InitLazy(
        internal_->glContext, Graphic3d_Vec2i(newRenderWidth, newRenderHeight),
  ...
  if (std::abs(internal_->scaleFactor - menu_scaling) > 0.0001) {
    ...
    internal_->viewCube->SetTransformPersistence(new Graphic3d_TransformPers(
        Graphic3d_TMF_TriedronPers, Aspect_TOTP_RIGHT_LOWER,
        Graphic3d_Vec2i((int)(85 * internal_->scaleFactor),
...
void ViewController::configureHighlightStyle(const Handle(Prs3d_Drawer) &
                                             theDrawer) {
  Handle(Prs3d_ShadingAspect) shadingAspect = new Prs3d_ShadingAspect();
  shadingAspect->SetColor(internal_->highlightColor);
  shadingAspect->SetMaterial(Graphic3d_NOM_PLASTIC);
