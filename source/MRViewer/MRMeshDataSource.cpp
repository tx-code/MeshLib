...
IMPLEMENT_STANDARD_RTTIEXT(MR_MeshDataSource, MeshVS_DataSource);
using namespace MR;

struct MR_MeshDataSource::InternalMeshData
{
  InternalMeshData(const Mesh& theMesh)
      : mesh(theMesh)
  {
  }
...
MR_MeshDataSource::MR_MeshDataSource(const Mesh&                           theMesh,
                                     const std::optional<MR::FaceNormals>& theFaceNormals,
                                     const std::optional<MR::VertNormals>& theVertNormals)
    : data_(std::make_unique<InternalMeshData>(theMesh))
{
  const auto& validVerts = theMesh.topology.getValidVerts();
  const auto& validFaces = theMesh.topology.getValidFaces();

  bool needComputeNormals = !theFaceNormals.has_value() || !theVertNormals.has_value()
                            || theFaceNormals->size() != validFaces.size()
                            || theVertNormals->size() != validVerts.size();

  if (needComputeNormals)
  {
    data_->normals = computeMeshNormals(theMesh);
  }
  else
  {
    data_->normals = MeshNormals(theFaceNormals.value(), theVertNormals.value());
  }
...
MR_MeshDataSource::~MR_MeshDataSource() = default;

Standard_Boolean MR_MeshDataSource::GetGeom(const Standard_Integer ID,
                                            ...
                                            MeshVS_EntityType&     Type) const
{
  if (IsElement)
  {
    ...
    if (ID >= 0 && data_->mesh.topology.hasFace(FaceId(ID)))
    {
      Type    = MeshVS_ET_Face;
      ...
      Vector3f v0, v1, v2;
      data_->mesh.getTriPoints(FaceId(ID), v0, v1, v2);
  ...
  else
  {
    ...
    if (ID >= 0 && data_->mesh.topology.hasVert(VertId(ID)))
    {
      Type    = MeshVS_ET_Node;
      NbNodes = 1;

      // Get the vertex coordinates
      const Vector3f& point = data_->mesh.points[VertId(ID)];
...
Standard_Boolean MR_MeshDataSource::GetGeomType(const Standard_Integer ID,
                                                const Standard_Boolean IsElement,
                                                MeshVS_EntityType&     Type) const
{
  if (IsElement)
...
Standard_Boolean MR_MeshDataSource::GetNodeNormal(const Standard_Integer ranknode,
                                                  const Standard_Integer ElementId,
                                                  Standard_Real&         nx,
                                                  Standard_Real&         ny,
                                                  Standard_Real&         nz) const
{
  if (ElementId >= 0 && data_->mesh.topology.hasFace(FaceId(ElementId)) && ranknode >= 0
      && ranknode < 3)
  {
    // Get the three vertices of the face
    VertId verts[3];
    data_->mesh.topology.getTriVerts(FaceId(ElementId), verts);

    // Get the normal at the specified vertex
    if (ranknode < 3 && verts[ranknode].valid())
    {
      Vector3f normal = data_->normals.vertNormals[verts[ranknode]];
      nx              = normal.x;
      ny              = normal.y;
      nz              = normal.z;
      return Standard_True;
...
Standard_Boolean MR_MeshDataSource::GetNodesByElement(const Standard_Integer   ID,
                                                      ...
                                                      Standard_Integer&        NbNodes) const
{
  if (ID >= 0 && data_->mesh.topology.hasFace(FaceId(ID)))
  {
    // Get the three vertices of the face
    VertId verts[3];
    data_->mesh.topology.getTriVerts(FaceId(ID), verts);
...
Standard_Boolean MR_MeshDataSource::GetNormal(const Standard_Integer Id,
                                              ...
                                              Standard_Real& nz) const
{
  if (Id >= 0 && data_->mesh.topology.hasFace(FaceId(Id)))
  {
    ...
    Vector3f normal = data_->normals.faceNormals[FaceId(Id)];
    nx              = normal.x;
    ny              = normal.y;
    nz              = normal.z;
    return Standard_True;
...
Standard_Boolean MR_MeshDataSource::GetNormalsByElement(
  ...
  Handle(TColStd_HArray1OfReal)& Normals) const
{
  ...
  if (IsNodal)
  {
    ...
    for (Standard_Integer i = 0; i < nbElementNodes; ++i)
    {
      if (verts[i].valid())
      {
        Vector3f normal = data_->normals.vertNormals[verts[i]];
  ...
  else
  {
    ...
    Vector3f faceNormal = data_->normals.faceNormals[FaceId(Id)];
    for (Standard_Integer i = 0; i < nbElementNodes; ++i)
    {
...
Bnd_Box MR_MeshDataSource::GetBoundingBox() const
{
  if (data_->mesh.topology.numValidVerts() == 0)
  {
  ...
  MR::Box3f mrBox = data_->mesh.getBoundingBox();
  ...
  if (!mrBox.valid())
  {
    ...
    mrBox = data_->mesh.computeBoundingBox();
  }
